...
- Keys may be restricted by scopes (permissions). Keys created without stored scopes retain the legacy order API scopes only. New scopes, including `reports.invoices.read`, must be granted explicitly.
...
| `status` | optional | Order status — one of `open`, `approved`, `assigned`, `picked`, `pending_finance_approval`, `invoiced`, `in_transit`, `delivered`, `grn_entered`, `rto`. |
...
| `assigned` | Order assigned for picking or fulfillment |
| `picked` | Items picked from inventory |
| `pending_finance_approval` | Order is waiting for finance approval before invoicing |
...
FilFlo does not provide a separate public sandbox environment at this time. To test end-to-end, use a FilFlo-provided test workspace on the production backend:
...
3. Trigger a test order status change in the FilFlo test workspace
